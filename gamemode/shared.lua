...
LASTHUMANSOUNDLENGTH = SoundDuration(LASTHUMANSOUND)

function LoopLastHuman()
	surface.PlaySound(LASTHUMANSOUND)
	timer.Create("Last Human Loop", LASTHUMANSOUNDLENGTH, -1, function() 
		if not ENDROUND then 
			surface.PlaySound(LASTHUMANSOUND)
		end
	end )
	timer.Start("Last Human Loop")
end
